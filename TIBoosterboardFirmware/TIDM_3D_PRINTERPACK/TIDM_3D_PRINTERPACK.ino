...
int target_raw_0 = 0, target_raw_1 = 0;
int current_raw_0, current_raw_1;
      ...
      case 28: // G28 Homing
        ...
        if((home_all_axis) || (code_seen(axis_codes[X_AXIS]))) 
        {
          if ((X_MIN_PIN > -1 && X_HOME_DIR==-1) || (X_MAX_PIN > -1 && X_HOME_DIR==1))
          {
            ...
            y_steps_to_take = 0; z_steps_to_take = 0; e_steps_to_take = 0;
        ...
        if((home_all_axis) || (code_seen(axis_codes[Y_AXIS]))) 
        {
          if ((Y_MIN_PIN > -1 && Y_HOME_DIR==-1) || (Y_MAX_PIN > -1 && Y_HOME_DIR==1))
          {
            ...
            x_steps_to_take = 0; z_steps_to_take = 0; e_steps_to_take = 0;
        ...
        if((home_all_axis) || (code_seen(axis_codes[Z_AXIS]))) 
        {
          if ((Z_MIN_PIN > -1 && Z_HOME_DIR==-1) || (Z_MAX_PIN > -1 && Z_HOME_DIR==1))
          {
            ...
            x_steps_to_take = 0; y_steps_to_take = 0; e_steps_to_take = 0;
  ...
  if(code_seen('M'))
  {
    ...
    switch( (int)code_value() ) 
    {
      ...
      case 140:  //M140 set bed temp
        if (code_seen('S')) target_raw_1 = temp2analog(code_value());
        Serial.println("ok");
        break;
      ...
      case 92: // M92
        ...
        if(code_seen('E')) e_steps_per_unit = (float)code_value();
        Serial.println("ok");
        break;
    }
...
{
  current_raw_0 = analogRead(TEMP_0_PIN);                  // If using thermistor, when the heater is colder than targer temp, we get a higher analog reading than target, 
  current_raw_1 = analogRead(TEMP_1_PIN);
  if(USE_THERMISTOR){
    current_raw_0 = max_adc_val - current_raw_0;   // this switches it up so that the reading appears lower than target for the control logic.
